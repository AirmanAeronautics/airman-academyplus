...
export const roleWidgets: Partial<Record<UserRole, RoleConfig>> = {
  ADMIN: {
    role: "ADMIN",
    widgets: [
      {
...
export const getAIAgentsForRole = (role: UserRole): string[] => {
  return roleWidgets[role]?.aiAgents || []
}

export const getPrimaryMetricsForRole = (role: UserRole): string[] => {
  return roleWidgets[role]?.primaryMetrics || []
}

export const isWidgetAvailableForRole = (widgetId: string, role: UserRole): boolean => {
