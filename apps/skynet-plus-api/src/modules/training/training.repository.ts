...
export class TrainingRepository {
  ...
  async listPrograms(tenantId: string, activeOnly: boolean = true): Promise<TrainingProgramRecord[]> {
    if (activeOnly) {
  ...
  async createLesson(
    ...
    const row = await this.databaseService.queryOne<TrainingLessonRecord>(
      `
      INSERT INTO training_lessons (tenant_id, program_id, code, name, lesson_type, sequence_order, duration_minutes, requirements)
  ...
  async createOrUpdateStudentProfile(
    ...
    const existing = await this.getStudentProfileByUserId(tenantId, userId);
  ...
  async recordLessonAttempt(
    ...
    // Get next attempt number if not provided
    ...
    const row = await this.databaseService.queryOne<StudentLessonAttemptRecord>(
      ...
      INSERT INTO student_lesson_attempts (
        tenant_id, student_profile_id, program_id, lesson_id,
        instructor_user_id, attempt_number, delivery_type, status,
        grade, remarks, scheduled_at, completed_at
      )
  ...
  async listStudentProgress(
    ...
    if (programId) {
      return this.databaseService.query<StudentLessonAttemptRecord>(
        ...
        ORDER BY lesson_id, attempt_number DESC
        `,
    ...
    return this.databaseService.query<StudentLessonAttemptRecord>(
      ...
      ORDER BY program_id, lesson_id, attempt_number DESC
      `,
